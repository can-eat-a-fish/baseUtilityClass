...
public class FestivalUtil {
	...
	public FestivalUtil(Date date) throws ParseException {
	 ...
	 */
	@SuppressWarnings("unused")
	private void getChinaCalendarMsg(int year, int month, int day) {

		int d = 7;

		if (((month) == 1) && day <= d) {
			System.out.println("春节");
		} else if (((month) == 1) && (day <= 15 && day >= 15 - d)) {
			System.out.println("元宵");
		} else if (((month) == 5) && (day <= 5 && day >= 5 - d)) {
			System.out.println("端午");
		} else if (((month) == 8) && (day <= 15 && day >= 15 - d)) {
			System.out.println("中秋");
		} else {
			if (month == 12) {
				if ((((monthDays(year, month) == 29) && day >= 29 - d))
						|| ((((monthDays(year, month) == 30) && day >= 30 - d)))) {
