...
import com.dongkesoft.iboss.model.LocationBean;

/* 
 * Copyright(c) 2016 dongke All rights reserved. / Confidential
 * 类的信息：
 *		1.程序名称： BaiduMapUtilByRacer
 *		2.功能描述：百度地圖工具類
 * 编辑履历：
 *		作者				日期					版本				修改内容
...
public class BaiduMapUtilByRacer {
	...
	public static void locateByBaiduMap(Context mContext, int time,
		...
		if (option == null) {
			...
			option.setScanSpan(time);//设置扫描间隔
	...
	public static class MyLocationListenner implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			mLocationBean.setStreetNum(location.getStreetNumber());
	...
	public static boolean NaviByBaidu(double mLat1, double mLon1, double mLat2,
		...
		if ((Double) mLat1 instanceof Double
			...
			// para.endName = "到这里结束";
			NaviParaOption para = new NaviParaOption().startPoint(pt1)
					.endPoint(pt2).startName("从这里开始").endName("到这里结束");
			try {
